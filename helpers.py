...
def build_graph(img):
    """Builds a weighted graph from a grid of black/white squares.
    ...
    for i in range(x_dim):
        for j in range(y_dim):
            ...
            if j < y_dim-1:
                G.add_edge((i,j),(i,j+1), weight=abs(img[i][j]-img[i][j+1]))
            if i < x_dim-1:
                G.add_edge((i,j),(i+1,j), weight=abs(img[i][j]-img[i+1][j]))
...
def draw_solution(lut, G):
    ...
    plt.clf()
    pos = nx.get_node_attributes(G, 'pos')
    color_map = ['white' if G.nodes[a]['color'] == 0 else 'black' for a in G.nodes]
    nx.draw_networkx_nodes(G, pos, node_color=color_map, edgecolors='black')
    nx.draw_networkx_edges(G, pos, edgelist=uncut_edges, style='dashdot', alpha=0.1, width=3)
    nx.draw_networkx_edges(G, pos, edgelist=cut_edges, style='solid', width=3, edge_color='red')
    labels = nx.get_edge_attributes(G,'weight')
    nx.draw_networkx_edge_labels(G,pos,edge_labels=labels)
...
def draw_boundary(lut, G, img):
    ...
    for i in range(3):
        for j in range(3):
            ...
            r = plt.Rectangle((left, bottom), width, height,
